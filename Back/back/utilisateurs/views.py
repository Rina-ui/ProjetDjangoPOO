...
class RegisterView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Créer automatiquement le profil Proprietaire si rôle PROPRIETAIRE
        if user.role == 'PROPRIETAIRE':
            Proprietaire.objects.get_or_create(utilisateur=user)
...
class Verify2FAView(APIView):
    ...
    def post(self, request):
        ...
        print(f"[2FA VERIFY] user={user.username}, code='{code}', secret='{user.totp_secret}', has_secret={bool(user.totp_secret)}")

        if not user.totp_secret:
            return Response({'error': 'Setup 2FA d\'abord'}, status=400)

        result = verifier_code(user.totp_secret, code)
        print(f"[2FA VERIFY] verifier_code result={result}")
