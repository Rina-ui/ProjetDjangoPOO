...
const WS_URL   = (import.meta.env.VITE_WS_URL || "http://localhost:8000").replace("http", "ws")
    ...
    const connectGlobalWS = () => {
        try {
            ...
            ws.onclose = () => {
                ...
                if (localStorage.getItem("access_token") && wsRetriesRef.current < WS_MAX_RETRIES) {
                    setTimeout(connectGlobalWS, 5000 * wsRetriesRef.current)
                }
            }
            ws.onerror = () => ws.close()
            ws.onmessage = (e) => {
